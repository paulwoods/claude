...
> This document defines the coding standards, conventions, and architecture patterns for this React + TypeScript application. All contributors — human or AI — must follow these rules when writing, reviewing, or modifying frontend code.
...
Never use class components. Always use **fat arrow (`const`) style** for component definitions. Never use `function` declarations or `React.FC` for components.
...
- Use `interface` for object shapes (props, API responses, models) — they are extendable and produce clearer error messages
...
Always use **Axios** for all HTTP requests. Never use the native `fetch` API. Axios provides better error handling, request/response interceptors, and automatic JSON parsing.
...
Do not use `styled-components`, `Emotion`, or inline `style` props for layout. Inline styles are only acceptable for dynamic values that cannot be expressed with Tailwind (e.g., dynamic pixel values from JS calculations).
...
*This document should be kept up to date as the project evolves. When introducing a new pattern or library, update this guide first.*
